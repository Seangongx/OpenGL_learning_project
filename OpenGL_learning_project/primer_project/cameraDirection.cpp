...
float pitch = 0.0;//俯仰角
float yaw = 270;//偏航角（也可以为-90）因为0.0的偏航角指向正右，所以我们这里有改动
float fov = 120;//视场角(设置大视场角挺有意思)
...
{
	if (firstMouse)//好像没有解决第一次进入跳跃的问题
	{
		lastX = xpos;
		lastY = ypos;
		firstMouse = false;
...
int main()
{
	...
	glfwSetCursorPosCallback(window, mouse_callback);//鼠标光标
	glfwSetScrollCallback(window, scroll_callback);//鼠标滑轮
