...
int main()
{
	...
	Shader lightShader("shaders/lightingMaps.vs", "shaders/lightingMaps.fs");
	...
	while (!glfwWindowShouldClose(window))//check the exit request
	{
		...
		lightShader.use();
		//lightShader.setVec3("objectColor", 1.0f, 0.5f, 0.31f);//改用结构体控制
		//lightShader.setVec3("lightColor", 1.0f, 1.0f, 1.0f);//改用结构体控制
		lightShader.setVec3("light.position", lightPos);//不是lightPos了，不更改的话会出现奇怪的效果，因为light没有获得position值
		lightShader.setVec3("viewPos", ourCamera.Position);

		lightShader.setVec3("material.ambient", 1.0f, 0.5f, 0.31f);//和物体颜色一致
		lightShader.setVec3("material.diffuse", 1.0f, 0.5f, 0.31f);//和物体颜色一致
