...
int main()
{
	...
	Shader lightShader("shaders/colors.vs", "shaders/colors.fs");
	...
	while (!glfwWindowShouldClose(window))//check the exit request
	{
		...
		lightShader.use();
		glUniform3f(glGetUniformLocation(lightShader.ID, "objectColor"), 1.0f, 0.5f, 0.31f); // 手动设置
		lightShader.setVec3("lightColor", 1.0f, 1.0f, 1.0f); // 或者使用着色器类设置
